...
export const checkRateLimit = (req: any, url: any) => {
  let rateLimited =
    req.statusCode === 429 ||
  ...
  if (rateLimited) {
    ...
    if (retryAfter) {
      ...
      if (retryAfter > config.rateLimitCap) {
        console.log(
          `Delay higher than rateLimitCap, setting it to ${config.rateLimitCap} seconds instead`
        );
      ...
      console.log(
        `I am temporarily ratelimited at ${url} (no ETA given, waiting ${config.rateLimitBackoff}s)`
      );
    }
...
export const isRateLimited = (url: any) => {
