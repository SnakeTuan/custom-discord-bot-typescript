...
export const loadConfig = (filename = "val-config.json") => {
  let loadedConfig: any;
  //   if (!process.env.TOKEN) {
  //     return console.error("You forgot to put discord token in .env!");
  //   }
  ...
  try {
    ...
    return console.error(`Could not JSON parse ${filename}! Is it corrupt?`, e);
  }
  ...
  if (
    (loadedConfig.HDevTokenAlert && !loadedConfig.HDevToken) ||
    loadedConfig.HDevToken === ""
  ) {
    console.error("Looks like you didn't put a HDevToken in config.json!");
    console.error(
      "The /profile command won't work without one. To get a key, see https://discord.gg/B7AarTMZMK"
    );
    console.error(
      "If you don't want to see this notification again, set HDevTokenAlert to false in config.json"
    );
  }
  ...
  applyConfig(loadedConfig, "token", process.env.DISCORD_TOKEN);
...
export const saveConfig = (filename = "val-config.json", configToSave: any) => {
  fs.writeFileSync(filename, JSON.stringify(configToSave || config, null, 2));
};

const applyConfig = (loadedConfig: any, name: string, defaultValue: any) => {
  if (loadedConfig[name] === undefined) {
